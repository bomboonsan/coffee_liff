    ...
    <main id="app" class="w-100 bg-white text-print py-2 px-2 text-black">
        ...
        <div class="row text-black">
            ...
            <div class="col-12 text-print-md">
                เมนู : <strong> {{ $order[0]->order_name }} [ {{ $order[0]->order_option }} {{ $order[0]->sweet_level }} ] </strong>
            </div>
            <div class="col-12 text-print-md">
                ชื่อลูกค้า : {{ $order[0]->name }}
            </div>
        </div><!--row-->
        
        <small class="w-100 px-1">
            <p class="text-print-md">
                <strong class="text-black">เพิ่มเติม</strong> : {{ $order[0]->order_note }}
            </p>
        </small>
    </main>
    <div class="d-print-none">
        <div class="text-center">
            <a class="btn btn-dark" href="{{url('/order/show')}}" role="button">กลับไปที่หน้ารายการ</a>
        </div>        
    </div>
